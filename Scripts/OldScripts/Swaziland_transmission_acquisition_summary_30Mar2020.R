...
reporthivbyageandgender$AgeCat <- cut(reporthivbyageandgender$Age, c(15,20,25,30,35,40,45,50,55,60,100), right=F)
...
getProportionInfectedByGroup <- function(GroupVars, PropVar, arg3=2, ...) {
  newVar <- sin(arg1) + sin(arg2)  # do Some Useful Stuff
  newVar / arg3   # return value 
}

GroupVars=c("scenario", "sim.id", "src.gender", "year")
PropVar=""

#1) How has the percent of PLHIV in different stages changed over time?
...
ggplot(subset(transm.risk.table.cohort.freq, scenario=="baseline" & src.stage.f=="Acute" & year>2014)) + 
