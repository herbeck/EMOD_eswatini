...
nrow(network.data.m[is.na(network.data.m$acq.year)==T & network.data.m$sim.id=="005a9663-9a43-ea11-a2c3-c4346bcb1551",]) #number of individuals who's infection was seeded in for sone sim (see campaign.json)
...
network.data.m$era[network.data.m$transm.year.int < 2016] <- "Pre-UTT"
network.data.m$era.acq <- "UTT era"
network.data.m$era.acq[network.data.m$transm.year.int < 2016] <- "Pre-UTT"
...
transmission.table <- subset(network.data.m, src.age.int>14 & src.age.int < 80)  %>%
...
#################################################################################################
#Incidence by ART covearge %
#################################################################################################

head(reporthivbyageandgender)
table(reporthivbyageandgender$src.age.cat)
detach(package:plyr)

#% of total population with unsuppressed viral load (e.g. not on ART)
art.fraction <- subset(reporthivbyageandgender, Age>14 & Age < 60)  %>%
  group_by(scenario, sim.id, Year2, Gender, IP_Key.ARTstate) %>%
  summarise(pop = sum(Population), inf=sum(Infected)) %>%  mutate(totpop = sum(pop), totinf=sum(inf))  %>% 
  mutate(totviremic = totinf-inf) %>%
  subset(IP_Key.ARTstate=="OnART") %>%
  mutate(ppdv=totviremic / totpop, cvl=totviremic/totinf) %>%
  group_by(scenario, Gender, Year2) %>%
  summarize(ppdv.med = quantile(ppdv, probs = 0.5, na.rm=T),cvl.med = quantile(cvl, probs = 0.5, na.rm=T))
art.fraction$Gender.vls <- art.fraction$Gender #code gender according to actual gender of VLS measurement

#Change in VLS by year
ggplot(subset(art.fraction, scenario=="baseline" & Year2>1990 & Year2<2065),aes(x=Year2)) + 
  geom_line(aes(y=ppdv.med*100, color=factor(Gender.vls)), size=2) +
  geom_point(aes(y=ppdv.med*100, color=factor(Gender.vls)), size=2) +
  xlab("") + ylab("%") +
  facet_wrap(~scenario) +
  ggtitle("Untreated fraction (HIV+ and HIV-)") +
  scale_x_continuous(expand = c(0,0)) + scale_y_continuous(expand = c(0,0), limits=c(0,100), breaks = seq(0,100,10)) +
  scale_color_manual(values = c("blue", "red"),labels = c("Male", "Female"))+
  scale_fill_manual(values = c("blue", "red"),labels = c("Male", "Female"))+
  coord_cartesian(ylim=c(0,30), xlim=c(1990,2055)) +
  theme(text=element_text(size=24))+
  theme(strip.background = element_rect(colour="white", fill="white")) +
  theme(axis.text.x = element_text(angle = 90)) +
  theme(legend.title=element_blank(),axis.line = element_line(colour = "black"),legend.position = "bottom")

setwd("C:\\Users\\aakullian\\Dropbox (IDM)\\Manuscripts\\Ongoing Manuscripts Folder\\Transmission in Era of UTT\\Figures")
ggsave("ppdv_by_year.jpg", height=10, width=10)

ggplot(subset(art.fraction, scenario=="baseline" & Year2>1990 & Year2<2065),aes(x=Year2)) + 
  geom_line(aes(y=cvl.med*100, color=factor(Gender.vls)), size=2) +
  geom_point(aes(y=cvl.med*100, color=factor(Gender.vls)), size=2) +
  xlab("") + ylab("%") +
  facet_wrap(~scenario) +
  ggtitle("Untreated fraction (HIV+)") +
  scale_x_continuous(expand = c(0,0)) + scale_y_continuous(expand = c(0,0), limits=c(0,100), breaks = seq(0,100,10)) +
  scale_color_manual(values = c("blue", "red"),labels = c("Male", "Female"))+ scale_fill_manual(values = c("blue", "red"),labels = c("Male", "Female"))+
  coord_cartesian(ylim=c(0,102), xlim=c(1990,2055)) +
  theme(text=element_text(size=24))+
  theme(strip.background = element_rect(colour="white", fill="white")) +
  theme(axis.text.x = element_text(angle = 90)) +
  theme(legend.title=element_blank(),axis.line = element_line(colour = "black"),legend.position = "bottom")
...
setwd("C:\\Users\\aakullian\\Dropbox (IDM)\\Manuscripts\\Ongoing Manuscripts Folder\\Transmission in Era of UTT\\Figures")
ggsave("cvl_by_year.jpg", height=10, width=10)

art.fraction$Gender = (art.fraction$Gender-1)*-1 #recode gender as opposite sex

incidence <- subset(reporthivbyageandgender, Age>14 & Age < 60)  %>%
  group_by(scenario, sim.id, Gender, Year2) %>%
  summarise(totuninf = sum(Uninfected.Population), totnewinf=sum(Newly.Infected)) %>%  mutate(inc = totnewinf/totuninf)  %>% 
  group_by(scenario, Gender, Year2) %>%
  summarize(inc.med = quantile(inc, probs = 0.5, na.rm=T),lb = quantile(inc, probs = 0.025, na.rm=T),ub = quantile(inc, probs = 0.975, na.rm=T))

inc.art.fraction <- merge(art.fraction, incidence, by=c("scenario","Year2", "Gender"))
summary(inc.art.fraction$cvl.med)
summary(inc.art.fraction$ppdv.med)

inc.art.fraction$era <- "2. ART"
inc.art.fraction$era[inc.art.fraction$Year2 < 2004] <- "1. PreART"
inc.art.fraction$era[inc.art.fraction$Year2 > 2015] <- "3. UTT"
table(inc.art.fraction$era, inc.art.fraction$Year2)

#Baseline - proportion of incidence
  ggplot(subset(inc.art.fraction, scenario=="baseline" & Year2>1985 & Year2<2050),aes(x=cvl.med)) + 
  #geom_line(aes(y=inc.med*100, color=factor(Gender)), size=2) +
  geom_point(aes(y=inc.med*100, color=factor(Gender)), size=2) +
  geom_errorbar(aes(ymin=lb*100, ymax=ub*100, color=factor(Gender))) +
  xlab("Untreated fraction (HIV+)") + ylab("Incidence (per 100 py)") +
  ggtitle("HIV incidence by untreated fraction (HIV+ and HIV-)") +
  #scale_colour_gradientn(colours=rainbow(3),  guide="legend", breaks=seq(2000,2050,5)) +
  #scale_fill_gradientn(colours=rainbow(3),  guide="legend") +
  facet_grid(~era) +
  scale_x_continuous(expand = c(0,0), trans="log",breaks=c(0.01,0.02,0.05,0.1, seq(0.2,1,0.2))) +
  scale_y_continuous(expand = c(0,0), trans="log", breaks=c(0.1, seq(0.5,4,0.5))) +
  scale_color_manual(values = c("blue", "red"),labels = c("Male", "Female"))+
  scale_fill_manual(values = c("blue", "red"),labels = c("Male", "Female"))+
  coord_cartesian(ylim=c(0.1,5), xlim=c(0.01,1.2)) +
  theme(text=element_text(size=24))+
  theme(strip.background = element_rect(colour="white", fill="white")) +
  theme(axis.text.x = element_text(angle = 90)) +
...
setwd("C:\\Users\\aakullian\\Dropbox (IDM)\\Manuscripts\\Ongoing Manuscripts Folder\\Transmission in Era of UTT\\Figures")
ggsave("HIVincidence_cvl_era.jpg", height=10, width=15)

ggplot(subset(inc.art.fraction, scenario=="baseline" & Year2>1990 & Year2<2050),aes(x=ppdv.med*100)) + 
  geom_point(aes(y=inc.med*100, color=factor(Gender)), size=2) +
  geom_errorbar(aes(ymin=lb*100, ymax=ub*100, color=factor(Gender))) +
  #geom_ribbon(aes(ymin=lb*100, ymax=ub*100, fill=factor(Gender)), size=2, alpha=0.2) +
  xlab("proportion viremic (HIV+)") + ylab("Incidence (per 100 py)") +
  #scale_colour_gradientn(colours=rainbow(3),  guide="legend", breaks=seq(2000,2050,5)) +
  #scale_fill_gradientn(colours=rainbow(3),  guide="legend") +
  #facet_grid(~Gender,  labeller=labeller(Gender = labs)) +
  ggtitle("HIV incidence by detectable viral load in HIV+") +
  scale_x_continuous(expand = c(0,0), breaks=seq(0,1,0.1)*100) +
  scale_y_continuous(expand = c(0,0), trans="log", breaks=seq(0.5,4,0.5)) +
  scale_color_manual(values = c("blue", "red"),labels = c("Male", "Female"))+
  scale_fill_manual(values = c("blue", "red"),labels = c("Male", "Female"))+
  coord_cartesian(xlim=c(0, 1.1)*100, ylim=c(0.3,5)) +
  theme(text=element_text(size=24))+
  theme(strip.background = element_rect(colour="white", fill="white")) +
  #theme(axis.text.x = element_text(angle = 90)) +
  #annotate("segment", x=-Inf, xend=Inf, y=-Inf, yend=-Inf)+
...
setwd("C:\\Users\\aakullian\\Dropbox (IDM)\\Manuscripts\\Ongoing Manuscripts Folder\\Transmission in Era of UTT\\Figures")
ggsave("HIVincidence_CVL.jpg", height=10, width=10)
...
ggplot(subset(transm.risk.table.all.ci, scenario=="baseline"),aes(x=year)) + 
  ...
  #geom_line(data=subset(transm.risk.table.combined.ci, scenario=="baseline"), aes(x=year, y=MedianInc*100), size=2, color="black") +
