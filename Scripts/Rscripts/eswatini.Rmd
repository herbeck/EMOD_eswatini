...
data_dir <- "~/Dropbox/IDM_stuff/EMOD_eswatini" 

load(file.path(data_dir,"/InputFiles/inputfiles2.rda")) 
#brings in transmissionreport.csv and reporthivbyageandgender.csv
...
#JH:  Why merge on SRC_ID and not DEST_ID?
...
network.data.m <- network.data %>%    
  ...
  #creates age bins, e.g (15,16,17,18,19) = [15,20)
...
# for Jeff E. and HIV Modeling Consortium
# number of infections by recipient/destination gender 
# need to sum across all sim.ids per Year and for each Gender and age category
t1 <- network.data.m  %>%
  group_by(scenario, sim.id, Year, DEST_GENDER, dest.age.cat) %>%
  summarize(transmissions=n()) 
r1 <- subset(reporthivbyageandgender)  %>%
  group_by(scenario, sim.id, Year) %>%
  summarize(Uninfected.Population=sum(Uninfected.Population))
ttable.dest.gender <- merge(t1, r1, by=c("scenario","Year","sim.id"), all.x=T) %>%
  #mutate(inc = transmissions/Uninfected.Population) %>%
  group_by(scenario, Year, sim.id) %>%
  mutate(trans = transmissions) %>%
  group_by(scenario, Year, DEST_GENDER, dest.age.cat) %>%
  summarize(prop.med=median(trans), prop.lb = quantile(trans, probs = 0.025), prop.ub = quantile(trans, probs = 0.975)) 

#merge t1 and r1 to get age, gender, Year, transmissions, uninfected.population
ttable.dest.gender <- subset(ttable.dest.gender, scenario =="baseline")
ttable.dest.gender <- subset(ttable.dest.gender, Year > 1999 & Year < 2026)

write.table(ttable.dest.gender, file="output.ttable.dest.gender.txt")
