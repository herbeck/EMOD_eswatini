...
t.dest.gender <- merge(t1, r1, by=c("scenario","Year","sim.id"), all.x=T)
t.dest.gender <- subset(t.dest.gender, scenario =="baseline")
t.dest.gender <- subset(t.dest.gender, Year > 1999 & Year < 2026)
t.dest.gender <- t.dest.gender  %>%
  group_by(scenario, sim.id, Year, DEST_GENDER, dest.age.cat) %>%
  summarize(transmissions=n()) 
...
ttable.dest.gender$scenario <- NULL

write.table(ttable.dest.gender, file="output.1")
