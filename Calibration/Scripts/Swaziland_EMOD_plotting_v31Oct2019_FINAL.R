...
#This brings in a no VMMC scenario with ART
wd13 <- "C:\\Users\\aakullian\\Dropbox (IDM)\\HIV\\EMOD\\Swaziland_05June2018\\Scenarios_calibration_run3_v05Sep2018_newcampaign_FINAL\\Scenarios_old\\ScenariosOct\\noVMMC\\ReportHIVByAgeAndGender\\"
...
for (i in seq(1,250,1)){
  reporthivbyageandgender <- read.csv(f[i])
  reporthivbyageandgender$scenario <- "noVMMCwithART"
  reporthivbyageandgender$sim.id <- paste0(files[i])
  #reporthivbyageandgender <- reporthivbyageandgender[,c(1,3,7,8,9,10,11,12,22,23)]
  if (i==1){
    reporthivbyageandgender.master <- reporthivbyageandgender
...
reporthivbyageandgender.master.novmmc <- reporthivbyageandgender.master
names(reporthivbyageandgender.master.novmmc)
reporthivbyageandgender.master.novmmc <- reporthivbyageandgender.master.novmmc[ , -which(names(reporthivbyageandgender.master.novmmc) %in% c("Newly.Tested.Positive","Newly.Tested.Negative","Tested.Past.Year.or.On_ART", "Tested.Ever","Diagnosed","NodeId","HasIntervention.Traditional_MC.","Non_Program_MMC", "Program_VMMC"))]
...
#add in extra scenario (no VMMC but with ART)
names(reporthivbyageandgender.master.novmmc[,c(1,2,6,7,8,9,14,15)])
names(reporthivbyageandgender.master.final[,c(2,3,7,8,9,10,15,16)])
reporthivbyageandgender.master.final <- rbind(reporthivbyageandgender.master.novmmc[,c(1,2,6,7,8,9,14,15)], reporthivbyageandgender.master.final[,c(2,3,7,8,9,10,15,16)])
...
span = 0.2
table(trajectories_IRoverall$scenario)
trajectories_IRoverall$scenario_f <- ordered(trajectories_IRoverall$scenario, levels=c("ART100pct","noARTnoVMMC","noVMMCwithART","noART","909090","baseline"))
swaziland.inc <- ggplot() +
  ...
  scale_color_manual(labels = c("100% ART initiation","No interventions", "VMMC only","ART only (no VMMC)", "Age-targeted 90-90-90", "Baseline"), values = c("green", "red", "orange", "yellow", "purple", "blue")) +
  guides(fill = guide_legend(keywidth = 2, keyheight = 1)) +
...
ggplot() +
  geom_smooth(data=trajectory_ART_calib.master.plot, aes(x=Year, y=ART_coverage*100, group = factor(Agecat), color=factor(Agecat)), method="loess",span=0.1, se=F, size=0.8) +
  geom_smooth(data=trajectory_ART_calib.master.plot, aes(x=Year, y=ART_coverage*100, color="average"),linetype=2, method="loess",span=0.1, se=F, size=0.8) +
  geom_point(data=subset(artdata), aes(x=Year, y=ART_coverage*100,
                                       color=factor(Agecat)), size=1) +
  geom_errorbar(data = subset(artdata), aes(x=Year, ymin=lb*100, ymax=ub*100, color=factor(Agecat)), width=1, size=0.8) +
  facet_grid(scenario_f~Gender, labeller=labeller(Gender = labs, scenario_f = labs.scenario)) +
  theme(legend.position="bottom") +
  #geom_hline(aes(yintercept=81)) +
  ylab("ART Coverage (%)")+
  theme_bw(base_size=18) +
  scale_color_manual(labels = c("15-24", "25-34", "35-44","Mean of ages 15-44"), values = c("blue", "purple", "red","black")) +
