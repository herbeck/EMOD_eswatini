...
from site_Swaziland import SwazilandCalibSite
...
ingest_xlsm_filename = os.path.join('Data', 'calibration_ingest_form-Swaziland_ART_vars.xlsm')
...
sites = [ SwazilandCalibSite( # note that the internal site structure is a bit different from previous calibration sites
            analyzers=analyzers,
...
template_files_dir = os.path.join(dir_path, 'InputFiles_July18', 'Templates')
static_files_dir = os.path.join(dir_path, 'InputFiles_July18', 'Static')

demog = DemographicsTemplate.from_file( os.path.join(static_files_dir,'Swaziland_Demographics_With_Properties.json') )
...
cfg = ConfigTemplate.from_file(os.path.join(template_files_dir, 'config_calibration.json'))
...
optimtool = OptimTool(params,
    ...
    samples_per_iteration = 700, # 700 is real size, 10 is testing
    center_repeats = 10, # 10 is real size, 2 is testing
    rsquared_thresh = 0.5  # <-- Linear regression goodness of fit threshold.  Above this, regression is used.  Below, use best point.
)

calib_manager = CalibManager(
    name            = 'Swaziland_calibration_ART_run4_v26Sep2018_newcampaign', 
